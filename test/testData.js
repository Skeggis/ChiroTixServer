...
// async function generateSpeakers(amount){
//     for(let i =0; )
// }
...
const NORMAL_EVENT = generateEvent()
  ...
  const NEW_SPEAKERS = [{name:faker.name.findName().replace(/\'/g,"")}, {name:faker.name.findName().replace(/\'/g,"")}, {name:faker.name.findName().replace(/\'/g,"")}]
